...
const LLMPrompt: FC<NodeProps<LLMPromptNodeDataType>> = ({ data, selected, id }) => {
	const [
		textState,
		{ set: setText, reset: resetText, undo: undoText, redo: redoText, canUndo, canRedo },
	] = useUndo(data.prompt);
					...
					<div className="h-full flex flex-col gap-1 p-4 text-gray-900">
						...
						<div className="flex gap-1">
							{data.inputs.inputNodes.map((inputNode: CustomNode) => {
								const color = inputNode.type === 'textInput' ? 'emerald' : 'yellow';
								const colorClass = `bg-${color}-400 text-${color}-800`;
								return (
									<div key={inputNode.id}>
										<button
											type="button"
											// convert below to use color for both bg and text
											className={`rounded py-1 px-2 text-xs font-semibold text-white shadow-sm ${colorClass}`}
											onClick={() => {
