...
function allParentsVisited(nodes: CustomNode[], node: CustomNode, visited: Set<string>): boolean {
	const inputNodes = getNodes(nodes, node.data.inputs.inputs);
	return inputNodes.every((parent) => visited.has(parent.id));
}

export function getNextNode(state: TraversalStateType, nodes: CustomNode[]): string | null {
	while (state.stack.length > 0) {
		...
		if (state.visited.has(node.id) || test || state.skipped.has(node.id)) continue;
